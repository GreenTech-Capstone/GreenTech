...
export default function Dashboard({ navigation }) {
  ...
  const getCardColor = (key, value) => {
    ...
    let idealKey = key;
    if (key === 'ph') idealKey = 'ph_voltage';
    if (key === 'ec') idealKey = 'ec_voltage';
    if (key === 'water_level') idealKey = 'distance_cm';

    const range = ranges[idealKey] || { min: 0, max: 100 };
    if (numValue < range.min) return '#f1d623ff';
    if (numValue > range.max) return '#e91212ff';
    return '#dbf7c5';
  };
  ...
  const sendSensorDataToSupabase = async (sensorData) => {
    const { data, error } = await supabase
      ...
      .insert([{
        ph_voltage: sensorData.ph ?? 0,
        ec_voltage: sensorData.ec ?? 0,
        temperature: sensorData.temperature ?? 0,
        humidity: sensorData.humidity ?? 0,
        distance_cm: sensorData.water_level ?? 0,
    ...
    if (data && data.length > 0) {
      ...
      setParameters((prev) =>
        prev.map((p) => {
          ...
          switch (p.key) {
            case 'ph':
              value = latest.ph_voltage != null ? convertVoltageToPH(latest.ph_voltage).toFixed(2) : 'N/A';
              break;
            case 'ec':
              value = latest.ec_voltage != null ? latest.ec_voltage.toFixed(2) : 'N/A';
              break;
            case 'temperature':
              value = latest.temperature != null ? latest.temperature.toFixed(1) : 'N/A';
              break;
            case 'humidity':
              value = latest.humidity != null ? latest.humidity.toFixed(1) : 'N/A';
              break;
            case 'water_level':
              value = latest.distance_cm != null ? latest.distance_cm.toFixed(1) : 'N/A';
              break;
          }
      ...
      sendSensorDataToSupabase({
        ph: latest.ph_voltage ?? 0,
        ec: latest.ec_voltage ?? 0,
        temperature: latest.temperature ?? 0,
        humidity: latest.humidity ?? 0,
        water_level: latest.distance_cm ?? 0,
        pump_on: latest.pump_on ?? false,
      });
    }
  };
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.container}>
        {parameters.map((param) => (
          <View
            key={param.id}
            style={[styles.card, { backgroundColor: getCardColor(param.key, param.value) }]}
          >
      ...
      {visibleMenu !== null && (
        <Pressable style={styles.overlay} onPress={closeMenu}>
          <View
            style={[
              styles.menu,
              { position: 'absolute', top: menuPosition.top + 5, left: menuPosition.left - 120 },
            ]}
          >
            <Pressable
              ...
              onPress={() => {
                ...
                if (param)
                  navigation.navigate('Alerts', { paramKey: mapParamKeyForScreens(param.key), paramName: param.name });
              }}
            >
            ...
            <Pressable
              ...
              onPress={() => {
                ...
                const param = parameters.find(p => p.id === visibleMenu);
                if (param)
                  navigation.navigate('History', { paramKey: mapParamKeyForScreens(param.key), paramName: param.name });
              }}
            >
            ...
            <Pressable
              ...
              onPress={() => {
                ...
                const param = parameters.find(p => p.id === visibleMenu);
                if (param)
                  navigation.navigate('Range', { paramKey: mapParamKeyForScreens(param.key), paramName: param.name });
              }}
            >
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 15, padding: 16, marginVertical: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', elevation: 4 },
  ...
  menu: { backgroundColor: '#dbf7c5', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 10, elevation: 5, borderWidth: 1.5, borderColor: '#05542f' },
  menuBox: { backgroundColor: '#dbf7c5', borderWidth: 2, borderColor: '#05542f', borderRadius: 20, paddingVertical: 5, paddingHorizontal: 30, marginBottom: 10 },
